...
class Hospital:
    ...
    def prescription_data(self):
        ...
        if nameoftablets == '' or ref == '':
            ...
            try:
             my_cursor.execute('INSERT INTO hospital (nameoftablets, ref, dose, dob, lot, issuedata, expdate, dailydose, furtherinfo, storage, driving, prescription, nhsnumber, pname, adress, rooftablets, date) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)', (
             nameoftablets, ref, dose, dob, lot, issue_data, exp_date, daily_dose, further_information,nhs_number, patient_name, patient_address, datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
             conn.commit()
    ...
    def prescription(self):
       self.txtPrescription.insert(END,"name of tablets : "+self.Nameoftablets.get()+"n")
       self.txtPrescription.insert(END,"Reference No : "+self.ref.get()+"n")
       self.txtPrescription.insert(END,"Dose: "+self.Dose.get()+"n")
       self.txtPrescription.insert(END,"Number of tablets : "+self.numberoftablets.get()+"n")
       self.txtPrescription.insert(END,"Lot : "+self.Lot.get()+"n")
       self.txtPrescription.insert(END,"Issue Date : "+self.Issuedata.get()+"n")
       self.txtPrescription.insert(END,"Exo date : "+self.ExpDate.get()+"n")
       self.txtPrescription.insert(END,"Daily dose : "+self.DailyDose.get()+"n")
       self.txtPrescription.insert(END,"Side effect : "+self.Sideeffect.get()+"n")
       self.txtPrescription.insert(END,"Further information : "+self.Furtherinformation.get()+"n")
       self.txtPrescription.insert(END,"Storage advice :"+self.StorageAdvice.get()+"n")
       self.txtPrescription.insert(END,"DrivingUsingMachine :"+self.Drivingusinmachine.get()+"n")
       self.txtPrescription.insert(END,"Patient ID : "+self.patientid.get()+"n")
       self.txtPrescription.insert(END,"NHS number : "+self.Nhsnumber.get()+"n")
       self.txtPrescription.insert(END,"Patient Name : "+self.Patientname.get()+"n")
       self.txtPrescription.insert(END,"Date of Birthay : "+self.patientbirthday.get()+"n")
       self.txtPrescription.insert(END,"Patient adress : "+self.PatientAdress.get()+"n")
    def delete(self):
     conn = mysql.connector.connect(host='localhost', username='root', password='123456789', database='Mydata')
     my_cursor=conn.cursor()
